 ...
 */
  ...
  me_FDCAN_setRxInterrupt(2);
  me_FDCAN_setRxInterrupt(3);
}
...
void loop() 
{
  ...
  /*
   * fdCAN ports have 3 message FIFO
   *    polling the ports will empty buffers
   */
  checkMessages();
  delay(960);
}
...
void CAN2_RX_ISR()
{
  ...
  uint32_t id=0;
  uint8_t rxdata[8];
  int qty = meFDCAN_receive(&id,&len,rxdata,2);
  if (qty < 0)
  {
    return ; 
  }
...
void CAN3_RX_ISR()
{
  ...
  uint32_t id=0;
  uint8_t rxdata[8];
  int qty = meFDCAN_receive(&id,&len,rxdata,3);
  if (qty < 0)
  {
    return ; 
  }
...
{
  CAN2_RX_ISR();  //---  my handler 
  FDCAN2->IR |= FDCAN_FLAG_RX_FIFO0_NEW_MESSAGE; //--- clears interrupt
}

void FDCAN3_IT0_IRQHandler(void)
{
  CAN3_RX_ISR();  //---  my handler 
  FDCAN3->IR |= FDCAN_FLAG_RX_FIFO0_NEW_MESSAGE; //--- clears interrupt
}
